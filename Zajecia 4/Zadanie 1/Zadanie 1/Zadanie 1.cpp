...
enum
{
	...
	TEXTURE_madonna,                    // tekstura madonna
	TEXTURE_madonna_UNC,                // teksura madonna nieskompresowana
	TEXTURE_madonna_gray,               // tekstura madonna_gray
	TEXTURE_madonna_gray_UNC,           // tekstura madonna_gray nieskompresowana
...
GLuint madonna, madonna_UNC, madonna_gray, madonna_gray_UNC;
...
void GenerateTextures()
{
	...
	// wczytanie tekstury madonna.tga
	GLboolean error = load_targa("madonna.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku madonna.tga");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &madonna);

	// dowiązanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, madonna);

	// definiowanie tekstury z kompresją
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_RGB, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &madonna_UNC);

	// dowiązanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, madonna_UNC);
	...
	// wczytanie tekstury madonna_gray.tga
	error = load_targa("madonna_gray.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku madonna_gray.tga");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &madonna_gray);

	// dowiązanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, madonna_gray);

	// definiowanie tekstury z kompresją
	glTexImage2D(GL_TEXTURE_2D, 0, GL_COMPRESSED_LUMINANCE, width, height, 0, format, type, pixels);

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &madonna_gray_UNC);

	// dowiązanie stanu tekstury
	glBindTexture(GL_TEXTURE_2D, madonna_gray_UNC);
...
void Menu(int value)
{
	...
	// teksura madonna nieskompresowana
	case TEXTURE_madonna_UNC:
	{
		texture = madonna_UNC;
		DisplayScene();
	}
	...
	// tekstura madonna_gray
	case TEXTURE_madonna_gray:
	{
		texture = madonna_gray;
		DisplayScene();
	}
	break;

	// tekstura madonna_gray nieskompresowana
	case TEXTURE_madonna_gray_UNC:
	{
		texture = madonna_gray_UNC;
		DisplayScene();
	}
	break;
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("madonna.tga (kompresja)", TEXTURE_madonna);
	glutAddMenuEntry("madonna.tga (bez kompresji)", TEXTURE_madonna_UNC);
	glutAddMenuEntry("madonna_gray.tga (kompresja)", TEXTURE_madonna_gray);
	glutAddMenuEntry("madonna_gray.tga (bez kompresji)", TEXTURE_madonna_gray_UNC);
